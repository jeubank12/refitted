...
- **Database Migrations**: See `room/CLAUDE.md` for migration guidelines
- **Compose State**: See `ui/CLAUDE.md` for state management patterns
- **Repository Pattern**: See `app/CLAUDE.md` for offline-first implementation
...
- Default to no comment. Do not add comments explaining what code does when the code is self-explanatory — prefer naming and structure over prose. Only add one for a genuinely strong reason: a non-obvious *why* (a hidden constraint, a workaround, a subtle invariant) that the code cannot express on its own — never restate the *what*. Prefer putting that *why* in the commit message over a code comment when it's about the change itself rather than a lasting property of the code
- If a *why* is a reusable pattern or gotcha rather than something local to one call site (e.g. a Compose/Flow/Room quirk that'll bite the next call site too), it does not belong inline at all — document it once in the relevant module's `CLAUDE.md` instead
- Meaningful user-facing changes — new features, behavior changes, notable fixes a user would have consciously run into — should get an entry in `ui/src/main/res/values/changelog.xml` (a `string-array` where an item starting with `**` is a version header, e.g. `**v1.4.0`, and following items are bullets shown under it). Not every user-visible diff qualifies: small polish or bugfixes (e.g. a one-frame visual glitch, a subtle timing fix) aren't worth a bullet unless a user would actually notice and care. When in doubt, ask rather than assume it belongs. New entries go under the current in-progress version header at the top rather than bumping the version — the in-app dialog (`Changelog.kt`, gated in `UserViewModel.shouldShowChangelog()`) fires once per change in `versionCode` (`app/build.gradle`), not per changelog edit, so editing this file alone is safe and doesn't trigger anything for existing users until a release bumps the version. Keep entries short and benefit-oriented, not implementation detail. Order bullets within a version by importance/impact to the user, not chronologically by when they landed — reorder existing entries in that version if a new one outranks them.
